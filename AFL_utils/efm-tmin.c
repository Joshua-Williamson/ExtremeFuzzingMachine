...
u8 threshold_given = 0; /*Is the threshold given? JW*/
...
static void minimize(char** argv) {
   ...
   * BLOCK NORMALIZATION *
   ***********************/
  /*JW: No point in checking if smaller here... keeps size*/
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc,argv,"+i:o:f:m:t:l:B:xeqQV")) > 0)
    ...
    switch (opt) {
      ...
      /*JW: add threshold arg */
      case 'l':

        if (threshold_given) FATAL("Multiple - options not supported");
        threshold_given= 1;
