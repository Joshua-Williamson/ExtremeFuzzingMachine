...
class Edge_Data(Dataset):
    ...
    def process_bitmaps(self, initial_map = False):
        ...
        for ind,seed in enumerate(to_map):
            ...
            #the first map
            if first_entry : self.bitmap = np.empty( [ len(to_map), pad ] )
            ...
            if not initial_map: self.bitmap = np.vstack(  ( self.bitmap, np.in1d(self.label,seed_edges).astype('int') ) )
            #Write to the emptry array
            else : self.bitmap[ind] = np.in1d(self.label,seed_edges).astype('int') 
    ...
    def reduce_variable_files(self):
        ...
        for seed in (self.vari_seed_list + self.havoc_seed_list):
            if (seed not in self.mapped_seeds) and (seed not in self.reducing_blacklist):
